...
.ps-inner{width:100%;max-width:430px;min-height:100dvh;display:flex;flex-direction:column;padding:0 28px}
...
.ps-pronouns{display:flex;gap:8px;margin-bottom:24px}
...
export default function ParentSetup({ displayName, onComplete }: Props) {
  ...
  if (moment === 0) return (
    <div className="ps">
      ...
      <div className="ps-inner">
        {dots}
        <div className="ps-body" key="m0">
          <div className="ps-h">Children tell you the real thing right before sleep.</div>
  ...
  // Moment 1 — The Ritual Shown
  if (moment === 1) return (
    <div className="ps">
      <style>{CSS}</style>
      <div className="ps-inner">
        {dots}
        <div className="ps-body" key="m1">
          <div className="ps-label">Here's how it works</div>

          <div
            className="ps-cards-track"
            ref={trackRef}
            onScroll={handleCardScroll}
          >
            <div className="ps-card-item">
              <span className="ps-card-emoji">📖</span>
              <div className="ps-card-title">Story</div>
              <div className="ps-card-desc">A story born from their day</div>
            </div>
            <div className="ps-card-item">
              <span className="ps-card-emoji">🥚</span>
              <div className="ps-card-title">DreamKeeper</div>
              <div className="ps-card-desc">A companion that grows with them</div>
            </div>
            <div className="ps-card-item">
              <span className="ps-card-emoji">📸</span>
              <div className="ps-card-title">Night Card</div>
              <div className="ps-card-desc">A memory saved forever</div>
            </div>
          </div>

          <div className="ps-card-dots">
            {[0, 1, 2].map(i => (
              <div key={i} className="ps-card-dot" style={{
                background: i === activeCard ? 'var(--amber)' : 'rgba(255,255,255,.12)',
                transform: i === activeCard ? 'scale(1.3)' : 'scale(1)',
              }} />
            ))}
          </div>

          <button className="ps-btn ps-btn-amber" onClick={() => setMoment(2)}>
      ...
      <div className="ps-inner">
        ...
        <div className="ps-body" key="m3">
          ...
          <button
            ...
            disabled={childName.trim().length < 2 || !childAge || !childPronouns}
            onClick={() => setMoment(4)}
          >
            Continue &rarr;
          </button>
