...
export default function SleepSeed({
  ...
  useEffect(() => {
    ...
    sGet("nightcards", userId).then(async (s) => {
      const v1Cards: any[] = s?.items || [];

      // Strip base64 from v1
      const stripB64 = (c: any) => ({...c,
        photo: c.photo?.startsWith?.('data:') ? null : c.photo,
        childDrawing: c.childDrawing?.startsWith?.('data:') ? null : c.childDrawing,
      });
      const v1Cleaned = v1Cards.map(stripB64);
      if (v1Cards.some((c:any) => c.photo?.startsWith?.('data:') || c.childDrawing?.startsWith?.('data:'))) {
        sSet("nightcards", {items:v1Cleaned}, userId);
        console.log('[NC] Cleaned base64 from v1 localStorage');
      }

      // Reconcile: merge v1 cards into v2 (the key NightCardLibrary reads)
      if (userId && v1Cleaned.length > 0) {
        try {
          const v2Key = `ss2_nightcards_${userId}`;
          const v2Raw = localStorage.getItem(v2Key);
          const v2Cards: any[] = v2Raw ? JSON.parse(v2Raw) : [];
          // Strip base64 from v2 too
          const v2Cleaned = v2Cards.map(stripB64);
          // Find cards in v1 that are missing from v2
          const v2Ids = new Set(v2Cleaned.map((c:any) => c.id));
          const missing = v1Cleaned.filter((c:any) => c.id && !v2Ids.has(c.id));
          if (missing.length > 0) {
            // Merge missing v1 cards into v2
            const merged = [...v2Cleaned, ...missing.map((c:any) => ({
              id: c.id, userId: userId,
            ...
            console.log(`[NC] RECOVERED ${missing.length} night cards from v1 → v2 (total: ${merged.length})`);

            // Also push recovered cards to Supabase
            for (const card of missing) {
              try {
                const dbRow: any = {
                  id: card.id, user_id: userId,
                  hero_name: card.heroName || '', story_id: null,
                  story_title: card.storyTitle || '',
                  character_ids: card.characterIds || [],
                  headline: card.headline || card.storyTitle || '',
                  quote: card.quote || card.bondingAnswer || card.bondingA || '',
                  memory_line: card.memory_line || null,
                  bonding_question: card.bondingQuestion || card.bondingQ || null,
                  bonding_answer: card.bondingAnswer || card.bondingA || null,
                  gratitude: card.gratitude || null,
                  photo_url: (card.photo && !card.photo.startsWith('data:')) ? card.photo : null,
                  emoji: card.emoji || '🌙',
                  date: card.date || new Date().toISOString().split('T')[0],
                };
                // Pack enrichment fields into extra
                const packed: any = {};
                ...
                if (Object.keys(packed).length) dbRow.extra = JSON.stringify(packed);
                else dbRow.extra = card.extra || null;
                await supabase.from('night_cards').upsert(dbRow);
              } catch(e) { console.error('[NC] Recovery Supabase push failed for', card.id, e); }
            }
            console.log(`[NC] Pushed ${missing.length} recovered cards to Supabase`);
          } else if (v2Cards.length !== v2Cleaned.length || v2Cards.some((c:any) => c.photo?.startsWith?.('data:'))) {
            // No missing cards but still need to clean base64
            localStorage.setItem(v2Key, JSON.stringify(v2Cleaned));
            console.log('[NC] Cleaned base64 from v2 localStorage');
          }
        } catch(e) { console.error('[NC] Reconciliation error:', e); }
      }
  ...
  const downloadStory = async () => {
    ...
    await generateStoryPdf({
      ...
      pages: book.pages,
      isAdventure: book.isAdventure,
      setup_pages: book.setup_pages,
      path_a: book.path_a,
      path_b: book.path_b,
      nightCard: book.nightCard,
      creatureEmoji: companionCreature?.creatureEmoji,
      includeNightCard: v8rShareIncludeNightCard,
    });
  };
  ...
  const renderSSStoryPage = (pg: any, pgNum: number, refrain: string|undefined) => (
    <div className={`ss-page ss-sp${warmMode?' warm':''}`} key={`sp-${pgNum}`}>
      ...
      <div className="ss-sp-body" style={{background:v8rTextBg,position:'relative'}}>
        ...
        {renderV8rMoonDots()}
        <div style={{display:'flex',justifyContent:'center',marginBottom:8,position:'relative',zIndex:10}}>
          <button
        ...
        <div style={{position:'absolute',bottom:0,left:0,right:0,zIndex:88,background:'#0C1840',borderTop:'1px solid rgba(255,255,255,.09)',borderRadius:'24px 24px 0 0',animation:'v8r-shareReveal .36s cubic-bezier(.22,.8,.3,1) both'}}>
          ...
          <div style={{padding:'16px 22px 24px'}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:6,marginBottom:14}}>
              {shareBtn(
                <svg viewBox="0 0 24 24" width="20" height="20" fill="rgba(234,242,255,.7)"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>,
                'X',
                ()=>{const msg=v8rShareMessage||`${book?.title} — a bedtime story for ${book?.heroName}`;window.open('https://twitter.com/intent/tweet?text='+encodeURIComponent(msg)+`&url=${encodeURIComponent(storyUrl)}`,'_blank');},
                'rgba(234,242,255,.7)'
              )}
              {shareBtn(
                <svg viewBox="0 0 24 24" width="22" height="22" fill="rgba(66,103,178,.85)"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>,
                'Facebook',
              ...
              {shareBtn(
                <svg viewBox="0 0 24 24" width="22" height="22" fill="none" stroke="rgba(245,184,76,.65)" strokeWidth="1.8" strokeLinecap="round"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="3" y1="9" x2="21" y2="9"/><line x1="9" y1="3" x2="9" y2="21"/></svg>,
                'Share card',
        ...
        <div style={{width:'100%',display:'flex',flexDirection:'column',gap:8,animation:'nc-fadeUp .5s .35s ease both'}}>
          {!book?.nightCard ? (
            ...
            <div style={{display:'flex',gap:8}}>
              ...
              <button onClick={downloadStory} style={{flex:1,padding:'12px',borderRadius:14,border:'1px solid rgba(244,239,232,.12)',background:'rgba(244,239,232,.04)',color:'rgba(234,242,255,.5)',fontSize:12,fontWeight:600,fontFamily:"'Nunito',sans-serif",cursor:'pointer'}}>PDF</button>
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div className="app">
        ...
        {/* Old night card flow removed — see git history (commit e666194) */}
