...
${resolvedAdv ? advSchema : simpleSchema}`;
      ...
      const coverUrl = illoUrl(story.cover_prompt, seed, 640, 280, g);
      let bookData, allUrls;

      if(resolvedAdv && story.setup_pages){
        const sU = story.setup_pages.map((p,i) => illoUrl(p.illustration_prompt, seed+i+1, 480, 220, g));
        const aU = story.path_a.map((p,i) => illoUrl(p.illustration_prompt, seed+100+i, 480, 220, g));
        const bU = story.path_b.map((p,i) => illoUrl(p.illustration_prompt, seed+200+i, 480, 220, g));
        ...
        const pU = story.pages.map((p,i) => illoUrl(p.illustration_prompt, seed+i+1, 480, 220, g));
        allUrls = [coverUrl,...pU];
        bookData = {
